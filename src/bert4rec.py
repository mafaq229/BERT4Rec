...
class SublayerConnection(nn.Module):
    ...
    def forward(self, x: torch.Tensor, sublayer):
        ...
        )  # (based on harvard annotated transformer)
...
class BERT4Rec(nn.Module):
    ...
    def __init__(
        self,
        item_vocab_size: int = MODEL_CONFIG[
            "vocab_size"
        ],  # 3884 is the max mapped movie id which should be the minimum size of the embedding layer
        ...
        position_encoding = LearnedPositionalEncoding(
            self.max_seq_len, self.d_model, self.dropout
        )
        ...
        encoder_layer = EncoderLayer(
            self.d_model, c(attention), c(feed_forward), self.dropout
        )
    ...
    def forward(
        ...
        logits = F.linear(
            hidden, self.item_embeddings.lut.weight, self.output_bias
        )  # (B, S, vocab_size)
    ...
    def compute_loss(
        ...
        masked_loss = (
            loss * loss_mask_flat.float()
        )  # multiples loss by 0 for non-masked positions
...
if __name__ == "__main__":
    ...
    loss = model.compute_loss(
        sample["source"], sample["target"], sample["source_mask"], sample["target_mask"]
    )
